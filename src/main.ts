...
(async () => {
  ...
  fastify.get('/static/slearn_100_code_update_2.ipa', function (req, reply) {
    const stream = fs.createReadStream(path.resolve('./static/slearn_100_code_update_2.ipa'));
    reply.type("application/octet-stream").send(stream);
  })
  ...
  fastify.get('/static/slearn_1098_dev_release.apk', function (req, reply) {
    const filePath = path.resolve('./static/slearn_1098_dev_release.apk')
    const stat = fs.statSync(filePath);
    const stream = fs.createReadStream(filePath)
    stream.on('open', () => {
      reply.header('Content-Length', stat.size).type("application/octet-stream").send(stream);
