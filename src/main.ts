...
(async () => {
  ...
  fastify.get('/static/slearn_100_dev_release.apk', function (req, reply) {
    const stream = fs.createReadStream(path.resolve('./static/slearn_100_dev_release.apk'));
    reply.type("application/octet-stream").send(stream);
  })
  fastify.get('/static/slearn_1098_dev_release.apk', function (req, reply) {
    const stream = fs.createReadStream(path.resolve('./static/slearn_1098_dev_release.apk'));
    reply.type("application/octet-stream").send(stream);
  })
